...
class Switch_Function:
    ...
    def Switch_Change_Cmd(self):
        try:
            ...
            x1 = f'{self.device_hostname1}: {self.management_ip1} - Might be Offline, Unreachable or cannot inaccessible. Please check the device manually!\n'\
...
class Firewall_Function(Switch_Function):
    ...
    def Firewall_Show_Cmd(self: str) -> str:
        list_data1 = json.loads(self.command1)
        try:
            conn = ConnectHandler(**self.my_device1)
            conn.enable()
            send_command_response = conn.send_multiline(list_data1,
                                                        use_textfsm=False)
            conn.disconnect()
            return send_command_response
        except Exception:
...
class main_information():
    ...
    def __init__(self):
        ...
        self.v_password = str(dict_data_value['PASSWORD_HIDDEN'])
        self.v_secret = str(dict_data_value['SECRET_HIDDEN'])
...
if __name__ == "__main__":
    ...
    combine_codes = main_information()
    with open("3HTML_TESTING.html", 'w') as writefile:
        sure_change1 = input(f"are you sure you want to proceed with the command:\n\t {combine_codes.v_command}? PLEASE DOUBLE CHECK! (yes/no):")
        if sure_change1 == str('yes') or sure_change1 == str('YES') or sure_change1 == str('Y') or sure_change1 == str('y'):
            with ThreadPoolExecutor(max_workers=200) as executor:
                multi_threading_results = executor.map(combine_codes.code,
                                                        combine_codes.v_hostname_ip, timeout=5)
                test123 = executor.shutdown(wait=True, cancel_futures=True)
